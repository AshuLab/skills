...
Worktree isolation follows the same split: sequential draining and local mode keep today's one-worktree-per-epic model; a parallel batch instead uses up to 3 **lane** worktrees - slots, not per-slice throwaways. A lane is created once, then reused round after round: freed (parked on the epic branch, not removed) when its occupant's merge lands, reassigned to whichever slice the next round dispatches into it.
