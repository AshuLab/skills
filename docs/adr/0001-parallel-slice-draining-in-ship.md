...
Worktree isolation follows the same split: local mode keeps today's one-worktree-per-epic model; every github-mode drain, batch of one or more, instead uses up to 3 **lane** worktrees - slots, not per-slice throwaways, that persist across the whole drain instead of being torn down per occupant. Mechanics (creating, freeing, reusing a lane) are in `solve/skills/ship/WORKTREES.md` -> **Lane worktrees, for a parallel batch**.
