...
Parallel batch dispatch is additive and scoped: each round takes up to 3 of the currently-startable hub slices as concurrent delegated builds - even a lone startable slice goes through this path, so a freed lane can be reused. Local mode is excluded - claiming a ticket there is just opening its file, not atomic, so two concurrent claims on the same epic aren't safe. The draining agent still serializes the actual merge of each finished slice into the epic branch one at a time, never in parallel. A batch member that errors, is killed, or stays silent past a 2-hour wait bound (counted from its own dispatch) is treated the same as any other `outcome: "stopped"` - the drain never hangs on a subagent that won't report.
Worktree isolation follows the same split: local mode keeps today's one-worktree-per-epic model; every github-mode drain, batch of one or more, keeps that epic worktree and adds up to 3 **lane** worktrees alongside it - slots, not per-slice throwaways, that persist across the whole drain instead of being torn down per occupant. Mechanics (creating, freeing, reusing a lane) are in [WORKTREES.md](../../solve/skills/ship/WORKTREES.md) -> *Lane worktrees, for a parallel batch*.
