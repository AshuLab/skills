...
A parallel dependency graph - several [[hub-slice]]s startable at once, no blocker between them - compounds this: today they're still drained one after another even though nothing requires it, so time is lost on top of the context problem.
...
Parallelizing in local mode too, treating "no board" as license to run concurrent drains. Rejected: local claiming has no atomicity to lean on, so two subagents could both pick the same unclaimed slice - the existing single-local-drain limitation isn't solved by this change, so parallel batches don't extend into it.
