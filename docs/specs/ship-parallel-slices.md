...
Worktree isolation follows the same split: today's one-worktree-per-epic model holds for local mode; every github-mode drain, batch of one or more, instead uses up to 3 lane worktrees - slots, not per-slice throwaways, that persist across the whole drain instead of being torn down per occupant. Mechanics are in `solve/skills/ship/WORKTREES.md` -> **Lane worktrees, for a parallel batch**.
