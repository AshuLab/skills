...
- `plugin.json` (plugin root) - Antigravity manifest: `$schema`, `name`,
  `description` only. No version, nothing to keep in sync.
...
to that plugin's `.claude-plugin/plugin.json` `skills` array, and add it to the plugin's
`README.md` skill table.

When adding a new plugin: create `<plugin>/.claude-plugin/plugin.json`, `<plugin>/plugin.json` and
