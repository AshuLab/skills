...
Run once per repo, and only if you want a remote tracker.
No config file = local markdown, zero setup.
...
- **On GitHub Enterprise Server, check the server version too** - sub-issues need GHES 3.17+, but *relationships* (`--blocked-by`, which the whole dependency graph rests on) need **GHES 3.19+**. Under 3.19 the failure is quiet and lopsided: `--parent` works, `--blocked-by` doesn't, and you end up with slices that look published but carry no blocking edges. On a GHES host below 3.19, say so and use local mode instead of publishing a graph that isn't there.
