---
name: pre-check
description: Optional gate you run on a written spec or ticket before it advances - to confirm it still applies today and to do the operational prep (labels, milestone, and the handoff notes the next step needs). Checks freshness + redundancy against the current codebase; checks the fundamentals only if the artifact never went through sharpen. Run it when there's distance from when the idea was sharpened; skip it when you just sharpened and are building now.
---

# pre-check - optional freshness + prep gate

A gate you *choose* to run on something already written - a spec (the PRD/epic) or a ticket (a slice/issue) - before it advances.
...
- **Origin** - it didn't come from `sharpen` (someone else's issue, a handed-down ticket), so nothing has validated it yet.

Skip it when you just sharpened the idea and you're building it now.

## It does NOT re-litigate the fundamentals

If it came from `sharpen`, assume the fundamentals hold - `sharpen` settled them (grilling first via `follow:pushback` if the idea was raw).
Don't re-run that questioning.
`pre-check` looks only at what `sharpen` couldn't:
