---
name: to-spec
description: Turn the brief that sharpen left into a final PRD, and decide where each user story gets tested - the call that both to-tickets and tdd depend on. No new interview, just synthesis; if there's no brief yet, sharpen comes first. Publishes the PRD as the GitHub epic issue.
---
...
The PRD becomes the **epic issue** on GitHub - infer `owner/repo` from the git remote if `docs/agents/solve.md` doesn't already declare it:
