...
- **local** (or no config) → one file per slice at `.solve/tickets/<feature>/NNN-slug.md`;
  blocking stays textual in "Blocked by".
- **github** → one issue per slice, in **topological order** (blocker before blocked).
  First read the epic's inheritable props (`gh issue view <epic> --json milestone,labels`),
  then one call per slice:
  `gh issue create --title "<title>" --body-file <ticket> --label solve:ticket,solve:ready --parent <epic> --blocked-by <n,n> --milestone <epic's>`
  plus the epic's non-`solve:` labels. `--parent` = sub-issue, `--blocked-by` = real
  dependency (drop the textual "Blocked by"). One call per slice, no Projects v2.
  Throttle the loop — fast creation trips rate limiting.
