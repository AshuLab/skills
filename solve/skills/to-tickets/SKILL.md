---
name: to-tickets
description: Break a spec (or a plan) into vertical, agent-ready slices - small tickets an agent can pick up one at a time, each with a definition of done. Use it once you have a spec and want to split the work. Publishes each slice as a GitHub sub-issue with real dependency edges.
---
...
The `NNN` in **Blocked by** are draft indexes; at publish they resolve to each blocker's real `#<issue>` handle, which GitHub auto-links.
...
Read the epic's milestone **before creating anything** (`gh issue view <epic> --json milestone`), if it has one, and carry it onto every slice in the batch.
