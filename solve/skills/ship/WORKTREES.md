...
**Sizing the round**: before assigning anything, count free lanes - detached HEAD, per *Freeing a lane* below, means free; a named slice branch checked out means occupied, whether by an in-flight build or by an unresolved stopped member from an earlier round (*Freeing a lane* never runs for a stopped member, so its lane stays occupied until it resolves). The round's batch is capped at whichever is smaller - 3 currently-startable hub slices, or lanes currently free. A startable slice that doesn't fit waits for the next round, same as any overflow past 3.
