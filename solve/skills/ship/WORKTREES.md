...
A merge that hits a conflict (*Draining an epic*'s conflict-halts-the-queue rule) leaves its member's lane the same way - unparked, still on that member's branch - since this section only runs once a merge actually lands, and a conflicting one never does.
