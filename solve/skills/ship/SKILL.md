...
**A parallel batch (*Draining an epic*) adds one exception to "one per epic, never per slice"**: each batch member's subagent works in its own worktree, one per slice-in-flight, in addition to - not instead of - the epic's own worktree from the table above. Create each member's worktree before dispatching that member, and remove it once that member's merge lands (*Close the loop* step 4) - never before, and never reused for a later round. This applies only while a batch is in flight; every row in the table above, and the one-per-epic rule itself, is otherwise unchanged for sequential drains and local mode. [WORKTREES.md](./WORKTREES.md) has the path and the mechanics.
...
One slice at a time in local mode; in github mode, up to 3 at once when more than one is startable. Either way the only thing you pick is which.
...
**In github mode, when that set holds more than one hub slice** (no open blocker), take up to 3 of them, lowest-numbered first, as one batch instead of just the lowest-numbered one - still *of this epic*, still unclaimed. More than 3 startable at once still caps the batch at 3; the rest wait for the next round, found the same way once this batch is done. One startable hub slice, or local mode, isn't a batch - proceed exactly as before, straight into the lifecycle below.
Dispatch each batch member through *Delegate the build* concurrently, without claiming any of them first - one subagent per member, each resolved to its own worktree (*Pick the tree*). Each subagent claims its own slice as the first step of its own lifecycle (*Claim it*, unchanged), exactly like a single dispatch; the draining agent doesn't pre-claim members before dispatch.
Wait for every member's close-out report before merging anything. If any of them reports `outcome: "stopped"` (*Delegate the build*'s report contract), halt the batch and report it - don't merge any member, even ones that already reported `ready-to-merge` (conservative default). Only once every member reports `ready-to-merge` do you merge them into the epic branch, one at a time, in the order the reports arrived - never two merges in flight. Each merge is still *Close the loop*'s steps 4-6, unchanged, its conflict-stop policy included: a same-file conflict between two members' merges stops that merge and reports it, exactly as a single-slice conflict does today - it just fires more often, since batch members build without being sequenced against each other.
