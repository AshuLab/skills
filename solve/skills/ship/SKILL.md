...
In local there's no board, so claiming is just opening the ticket file - *nothing records it*. Two runs pointed at the same local epic will both build every slice; the drain's "unclaimed" test has nothing to read. If concurrency matters, one epic per run is the only guarantee - github mode is where claiming is real.
