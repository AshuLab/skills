...
A conflict on one member's merge halts the rest of the queue immediately - any ready-to-merge members not yet merged wait for the next round, same as a stopped member. Abort the failed merge (`git merge --abort`) before reporting, so the epic worktree is left clean, not mid-conflict, for whoever resolves it.
