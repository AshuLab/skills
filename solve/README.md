...
.solve/
  config.yml           ← setup: the tracker choice
  tickets/<feature>/   ← to-tickets: one markdown file per slice
```

Both `docs/` and `.solve/` are committed, versioned work — not scratch.
`.solve/` is dot-prefixed but holds the tracker config and, in local mode, the
tickets — so it's source of truth: don't gitignore it.
